---
title: Semana del 5 de octubre  
---

05/10 
: Introducción a los modelos mixtos
  : [html]()
    : Leer [Dixon (2016)](https://doi.org/10.4148/2475-7772.1474)

06/10 
: Conexión entre diseños experimentales y modelos mixtos
  : [html]()
    : Leer [Gelman (2005)](https://projecteuclid.org/journals/annals-of-statistics/volume-33/issue-1/Analysis-of-variancewhy-it-is-more-important-than-ever/10.1214/009053604000001048.full)
  
07/10
: Pensar un modelo jerárquico. Selección y diagnóstico de modelos
  : [html]()
    : Leer [Hooten & Hobbs (2025), cap. 10](https://www.amazon.com/Bayesian-Models-Statistical-Primer-Ecologists/dp/069125012X/ref=sr_1_1?dib=eyJ2IjoiMSJ9.Jwn6iceCmPC53kfcwBXrBgdN7oAp_txB97B_0N1tUYyhIzHQP2G8UKS9m73XSQaP.qj568U7uPb7DFvSo62LRk6meUCkNvQgtMINZvKRjCG0&dib_tag=se&qid=1788647666&refinements=p_27%3AMevin+B.+Hooten&s=books&sr=1-1)

08/10
: Recuperación de información intergrupos y casos de datos desbalanceados.
  : [html]()
    : Leer [Yates (1940)](https://onlinelibrary.wiley.com/doi/10.1111/j.1469-1809.1940.tb02257.x)

09/10
: Análisis de un caso integrador  
  : [html]()
    : Leer [Gelman and Hill]()

  
